...
describe('history scan cursor', () => {
  it('preserves the server snapshot across opaque continuations and exhaustion', () => {
    const first = advanceHistoryScanCursor(createHistoryScanCursor(), page('opaque-a'));
    const next = advanceHistoryScanCursor(first, page('opaque-b'));
  ...
  it('rejects repeated cursors, including a cycle through earlier pages', () => {
    const first = advanceHistoryScanCursor(createHistoryScanCursor(), page('opaque-a'));
    expect(() => advanceHistoryScanCursor(first, page('opaque-a'))).toThrow('did not advance');
    const second = advanceHistoryScanCursor(first, page('opaque-b'));
    expect(() => advanceHistoryScanCursor(second, page('opaque-a'))).toThrow('did not advance');
  });
