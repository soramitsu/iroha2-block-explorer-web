...
import type { SuccessfulFetchingStatus } from '@/shared/api/consts';
import { SUCCESSFUL_FETCHING_STATUS } from '@/shared/api/consts';

const BASE_URL = window.location.origin.toString() + '/api/v1';

type GetResult<T> = { status: SuccessfulFetchingStatus, data: T } | { status: 'error', response: Response };
type ResultWithStatus<T> = { status: SuccessfulFetchingStatus, data: T } | ErrorResponse;
...
async function get<T>(path: string, params?: Record<string, any>): Promise<GetResult<T>> {
  ...
  return {
    status: SUCCESSFUL_FETCHING_STATUS,
    data: await res.json(),
  };
}
...
export async function fetchAccounts(params?: AccountSearchParams): Promise<ResultWithStatus<Paginated<Account>>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Account).parse(res.data) };
...
export async function fetchAccount(id: AccountId): Promise<ResultWithStatus<Account>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Account.parse(res.data) };
...
export async function fetchAssets(params?: AssetSearchParams): Promise<ResultWithStatus<Paginated<Asset>>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Asset).parse(res.data) };
...
export async function fetchAsset(id: AssetId): Promise<ResultWithStatus<Asset>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Asset.parse(res.data) };
...
export async function fetchAssetDefinitions(
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(AssetDefinition).parse(res.data) };
...
export async function fetchAssetDefinition(id: AssetDefinitionId): Promise<ResultWithStatus<AssetDefinition>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: AssetDefinition.parse(res.data) };
...
export async function fetchNFTs(params?: NFTsSearchParams): Promise<ResultWithStatus<Paginated<NFT>>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(NFT).parse(res.data) };
...
export async function fetchNFTById(id: NftId): Promise<ResultWithStatus<NFT>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: NFT.parse(res.data) };
...
export async function fetchDomains(params?: DomainSearchParams): Promise<ResultWithStatus<Paginated<Domain>>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Domain).parse(res.data) };
...
export async function fetchDomain(id: string): Promise<ResultWithStatus<Domain>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Domain.parse(res.data) };
...
export async function fetchBlocks(params?: Partial<PaginationParams>): Promise<ResultWithStatus<Paginated<Block>>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Block).parse(res.data) };
...
export async function fetchBlock(heightOrHash: number | string): Promise<ResultWithStatus<Block>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Block.parse(res.data) };
...
export async function fetchNetworkMetrics(): Promise<ResultWithStatus<NetworkMetrics>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: NetworkMetrics.parse(res.data) };
...
export async function fetchPeersInfo(): Promise<ResultWithStatus<PeerInfo[]>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: PeerInfo.array().parse(res.data) };
...
export async function fetchTransactions(
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Transaction).parse(res.data) };
...
export async function fetchTransaction(hash: string): Promise<ResultWithStatus<DetailedTransaction>> {
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: DetailedTransaction.parse(res.data) };
...
export async function fetchInstructions(
  ...
  if (res.status === SUCCESSFUL_FETCHING_STATUS)
    return { status: SUCCESSFUL_FETCHING_STATUS, data: Paginated(Instruction).parse(res.data) };
